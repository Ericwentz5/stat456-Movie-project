...
genre_cult_heatmap <- movies_clean %>%
  unnest(genres_list) %>%
  count(genres_list, cult_classic) %>%
  mutate(cult_classic = ifelse(cult_classic == 1, "Cult", "Not Cult"))

ggplot(genre_cult_heatmap, aes(x = cult_classic, y = genres_list, fill = n)) +
  geom_tile(color = "white") +
  scale_fill_gradient(low = "white", high = "red") +
  labs(title = "Genre Frequency: Cult vs. Non-Cult Classics",
       x = "Movie Type", y = "Genre", fill = "Count") +
